...
.logo-area{text-align:center;padding:40px 0 20px}.logo-area svg{width:60px;height:60px}.logo-area h1{font-size:22px;font-weight:700;margin-top:8px;color:#1F2937}.logo-area p{font-size:13px;color:#6B7280;margin-top:4px}
...
<div class="page">
<div class="logo-area">
<svg viewBox="0 0 60 60" fill="none"><rect width="60" height="60" rx="14" fill="#4F46E5"/><path d="M30 12l-15 7.5v15L30 49.5 45 34.5v-15L30 12z" stroke="#fff" stroke-width="3" fill="none"/><path d="M25 30l4 4 7-8" stroke="#fff" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/></svg>
<h1>森码云</h1><p>企业级实人认证 · 免费注册</p>
</div>
<div class="card">
...
<div style="margin:24px 16px"><van-button round block type="primary" native-type="submit" :loading="loading">注册</van-button></div>
</van-form>
<div style="text-align:center;margin-top:8px"><van-button size="small" plain @click="goLogin">已有账号？立即登录</van-button></div>
</div>
</div>
<?php else: ?>
...
<el-form-item><el-button type="primary" native-type="submit" :loading="loading" size="large" style="width:100%">注册</el-button></el-form-item>
</el-form>
...
const{createApp,ref,reactive}=Vue;const app=createApp({setup(){
    ...
    const rules={username:[{required:true,message:'请输入用户名',trigger:'blur'},{min:3,message:'至少3个字符'}],password:[{required:true,message:'请输入密码',trigger:'blur'},{min:6,message:'至少6个字符'}]};
    ...
    async function onSubmit(){
        ...
        loading.value=true;
        try{const r=await axios.post('/user/register',form);if(r.data.code===200)window.location.href=r.data.data.redirect||'/user/center';else <?php echo $isMobile?'vant.showToast':'ElementPlus.ElMessage.error'; ?>(r.data.msg||'注册失败')}catch(e){<?php echo $isMobile?'vant.showToast':'ElementPlus.ElMessage.error'; ?>('网络错误')}loading.value=false}
    function goLogin(){window.location.href='/user/login'}
    return{form,rules,f,loading,onSubmit,goLogin};
}});
