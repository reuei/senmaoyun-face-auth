...
<div style="margin:12px 16px 0"><van-checkbox v-model="remember" icon-size="14px" style="font-size:12px;color:#6B7280">记住密码</van-checkbox></div>
<div style="margin:20px 16px"><van-button round block type="primary" native-type="submit" :loading="loading">登录</van-button></div></van-form>
<div style="text-align:center;margin-top:8px"><van-button size="small" plain @click="goRegister">没有账号？立即注册</van-button></div></div></div>
<?php else: ?>
...
const{createApp,ref,reactive,onMounted}=Vue;const app=createApp({setup(){
    const form=reactive({username:'',password:''});const remember=ref(false);
    ...
    async function onSubmit(){
        ...
        loading.value=true;try{const r=await axios.post('/user/login',form);if(r.data.code===200){if(remember.value){localStorage.setItem('smy_username',form.username);localStorage.setItem('smy_password',form.password)}else{localStorage.removeItem('smy_username');localStorage.removeItem('smy_password')}window.location.href=r.data.data.redirect||'/user/center'}else <?php echo $isMobile?'vant.showToast':'ElementPlus.ElMessage.error'; ?>(r.data.msg||'登录失败')}catch(e){<?php echo $isMobile?'vant.showToast':'ElementPlus.ElMessage.error'; ?>('网络错误')}loading.value=false}
    function goRegister(){window.location.href='/user/register'}return{form,remember,rules,f,loading,onSubmit,goRegister}}});
