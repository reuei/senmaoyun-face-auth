...
?><!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>管理员登录 - 森码云 v1.0.4</title>
...
.card{width:100%;max-width:400px;padding:44px 40px;background:#fff;border-radius:16px;box-shadow:0 20px 40px rgba(0,0,0,.08);text-align:center}
.card .logo{margin-bottom:20px}.card .logo svg{width:52px;height:52px}
.card h2{font-size:20px;font-weight:700;margin-bottom:4px}.card .sub{color:#6B7280;font-size:13px;margin-bottom:28px}
</style></head><body>
<div id="app"><div class="card"><div class="logo"><svg viewBox="0 0 52 52" fill="none"><rect width="52" height="52" rx="12" fill="#4F46E5"/><path d="M26 10l-13 6.5v13L26 42.5 39 29.5v-13L26 10z" stroke="#fff" stroke-width="3" fill="none"/><path d="M22 26l3 3 6-7" stroke="#fff" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/></svg></div><h2>森码云管理后台</h2><p class="sub">v1.0.4 · 管理员登录</p>
...
<script>
const{createApp,ref,reactive}=Vue;createApp({setup(){
    ...
    async function onSubmit(){
        const v=await f.value.validate().catch(function(){return false});if(!v)return;
        loading.value=true;
        try{const r=await axios.post('/admin/login',form);if(r.data.code===200)window.location.href=r.data.data.redirect;else ElementPlus.ElMessage.error(r.data.msg||'登录失败')}catch(e){ElementPlus.ElMessage.error('网络错误')}loading.value=false}
    return{form,rules,f,loading,onSubmit};
