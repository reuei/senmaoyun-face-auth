<?php
$token = $token ?? '';
?><!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>实人认证 - 森码云 v2.0.1</title>
...
<style>
:root{--el-color-primary:#4F46E5}*{margin:0;padding:0;box-sizing:border-box}body{font-family:system-ui,-apple-system,"Segoe UI",Roboto,"Noto Sans SC","PingFang SC","Microsoft YaHei",sans-serif;background:#F8FAFC;color:#1F2937;min-height:100vh;line-height:1.5}
...
.step-item{display:flex;align-items:center;gap:8px;flex:1}.step-dot{width:30px;height:30px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:600;border:2px solid #E2E8F0;background:#fff;color:#9CA3AF;flex-shrink:0}.step-dot.active{border-color:#4F46E5;background:#4F46E5;color:#fff}.step-dot.done{border-color:#10B981;background:#10B981;color:#fff}
...
.name-bar{display:flex;align-items:center;gap:7px;padding:10px 14px;background:#fff;border:1px solid #E2E8F0;border-radius:8px;font-size:13px;font-weight:500}
.agreement{max-height:55vh;overflow-y:auto;font-size:12px;line-height:1.8;color:#6B7280;padding:16px;background:#F8FAFC;border-radius:8px;margin-bottom:18px}
...
<div class="content">
<div v-if="step===1"><el-card><h2>实人认证服务协议</h2><div class="agreement"><h3>森码云实人认证服务协议</h3><p>更新日期：2026年8月8日</p><h4>一、总则</h4><p>1.1本服务是由森码云向您提供的基于人脸识别技术的身份真实性核验服务。</p><p>1.2您在使用本服务前，请务必仔细阅读并充分理解本协议的全部内容。</p><h4>二、服务内容</h4><p>2.1本服务通过采集您的面部生物特征信息，结合身份证件信息进行核验。</p><p>2.2认证流程包括：签署协议、身份信息录入、活体人脸识别、结果判定。</p><p>2.3认证过程中，您的摄像头将全程开启。</p><h4>三、信息收集与使用</h4><p>3.1我们将收集您的姓名、身份证号码、面部生物特征信息。</p><p>3.2上述信息仅用于本次认证目的，认证完成后默认24小时后自动删除。</p><p>3.3我们承诺对您的个人信息采取严格的加密存储措施（AES-256-GCM）。</p><h4>四、用户权利与义务</h4><p>4.1您保证所提供的身份信息真实、准确、完整，且为本人信息。</p><p>4.2您同意在认证过程中配合完成活体检测动作。</p><h4>五、隐私政策</h4><p>5.1我们严格遵守《中华人民共和国个人信息保护法》。</p><h4>六、人脸识别授权书</h4><p>6.1本人知悉并同意，森码云将采集本人的面部生物特征信息用于身份核验。</p></div>
<el-checkbox v-model="agreed">我已阅读并同意相关协议</el-checkbox><el-button type="primary" :disabled="!agreed" @click="nextStep" style="width:100%;margin-top:16px">下一步，填写身份信息</el-button></el-card></div>
...
<div v-if="step===3"><el-card><h2>人脸识别</h2><div class="camera-wrap"><video ref="video" autoplay playsinline muted></video><canvas ref="canvas" style="display:none"></canvas><div class="camera-overlay" v-if="currentAction"><div class="camera-prompt">{{actionText}}</div></div></div>
<div class="name-bar" style="margin-bottom:16px">👤 {{form.name}}</div>
<el-card><h3 style="margin-bottom:12px">活体检测进度</h3><div v-for="a in actions" :key="a" style="display:flex;align-items:center;gap:8px;padding:6px 0;font-size:13px;color:#9CA3AF" :style="{color:doneActions.includes(a)?'#10B981':currentAction===a?'#4F46E5':''}"><span v-if="doneActions.includes(a)">✓</span><span v-else>○</span>{{actionLabels[a]}}</div>
<el-progress :percentage="Math.round(doneActions.length/actions.length*100)" style="margin-top:12px"/></el-card></el-card></div>
...
const{createApp,ref,reactive,computed,onMounted,nextTick}=Vue;createApp({setup(){
    ...
    async function verifyIdcard(){idcardError.value='';verifying.value=true;try{const r=await axios.post('/api/idcard/verify',form);if(r.data.code===200){idcardOk.value=true;const r2=await axios.post('/api/face/init',{token:'<?php echo $token; ?>',name:form.name,id_card:form.idcard});if(r2.data.code===200){recordNo.value=r2.data.data.record_no;step.value=3;await nextTick();startCamera()}else idcardError.value=r2.data.msg}else idcardError.value=r.data.msg}catch(e){idcardError.value='网络错误'}verifying.value=false}
    async function startCamera(){try{stream=await navigator.mediaDevices.getUserMedia({video:{width:{ideal:640},height:{ideal:480},facingMode:'user'}});if(video.value)video.value.srcObject=stream;await new Promise(r=>setTimeout(r,1000));runActions()}catch(e){resultStatus.value='failed';resultMsg.value='无法访问摄像头';step.value=4}}
    async function runActions(){for(let i=0;i<actions.length;i++){currentAction.value=actions[i];await new Promise(r=>setTimeout(r,2500));const img=captureFrame();if(img){try{const r=await axios.post('/api/face/action',{record_no:recordNo.value,action_type:actions[i],image:img});if(r.data.code===200)doneActions.value.push(actions[i])}catch(e){}}await new Promise(r=>setTimeout(r,800))}currentAction.value='';const finalImg=captureFrame();if(finalImg){try{const r=await axios.post('/api/face/result',{record_no:recordNo.value,image:finalImg});resultStatus.value=r.data.code===200?'success':'failed';resultMsg.value=r.data.msg||''}catch(e){resultStatus.value='failed';resultMsg.value='网络错误'}}if(stream){stream.getTracks().forEach(t=>t.stop())}step.value=4}
