<?php require __DIR__ . '/layout.php'; admin_layout_start('控制台', 'dashboard'); ?>
...
(async function(){
    try{
        var r=await axios.get('/api/admin/stats');
        if(r.data.code===200){
            var s=r.data.data.stats,trend=r.data.data.trend;
            document.getElementById('statsBox').innerHTML=
                '<div class="card stat-card"><div class="l">今日认证</div><div class="v">'+s.today_total+'</div><div class="s">通过 '+s.today_success+' 次</div></div>'+
            ...
            chart.setOption({tooltip:{trigger:'axis'},grid:{left:40,right:20,top:10,bottom:30},xAxis:{type:'category',data:trend.map(function(t){return t.date})},yAxis:{type:'value'},series:[{name:'总认证',type:'line',data:trend.map(function(t){return t.total}),smooth:true,lineStyle:{color:'#4F46E5'},itemStyle:{color:'#4F46E5'}},{name:'通过',type:'line',data:trend.map(function(t){return t.success}),smooth:true,lineStyle:{color:'#10B981'},itemStyle:{color:'#10B981'},areaStyle:{color:'rgba(16,185,129,.1)'}}]});
        }
    }catch(e){}
})();
</script>
